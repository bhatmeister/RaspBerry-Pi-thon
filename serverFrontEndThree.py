...
class Home(Page):
    def __init__(self, *args, **kwargs):
        ...
        def serveConnect(IP,Port):
         ...
         welcomeText = tk.Label(self, text="Raspberry.py Client", font=("Helvetica",28)).grid(row = 0, column=1)

        # inputIp = tk.Entry(self,width = 20)
        # inputIp.insert(0,"Enter IP Address")

        # inputPort =tk.Entry(self, width=6).grid(row=1,column=4)
        # inputPort.insert(0,"12345")

        # connectButton = tk.Button(self, text="Connect", bg="Black",fg="White", width=7, command=lambda: serveConnect(IP.get(),Port.get()))
        # connectButton.grid(row=3,col = 1)
